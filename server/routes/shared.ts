      ...
      if (sharedExp.steps && sharedExp.steps.length > 0) {
        const insertStep = db.prepare(`
          INSERT INTO steps (
            ...
            is_sample_dependent, samples_per_batch, extra_duration_minutes, is_overnight,
            sub_protocol, sub_protocol_id, order_index, routine_name,
            routine_duration_days, routine_recurrence, routine_recurrence_days, created_at
          )
        ...
        for (const step of sharedExp.steps) {
          ...
          const sInfo = insertStep.run(
            ...
            step.is_sample_dependent ? 1 : 0, step.samples_per_batch || 1, step.extra_duration_minutes || 0,
            step.is_overnight ? 1 : 0, step.sub_protocol || '', mappedSpId, step.order_index,
